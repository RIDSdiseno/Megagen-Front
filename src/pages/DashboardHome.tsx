    ...
    const fetchDashboards = async () => {
      ...
      try {
        ...
        if (isAdmin || isSupervisor) {
          ...
          if (!resp.ok) throw new Error(t("No se pudo cargar dashboard comercial"));
        ...
        if (isBodeguero || isAdmin) {
          ...
          if (!respB.ok) throw new Error(t("No se pudo cargar dashboard bodega"));
          const dataB = (await respB.json()) as BodegaDashboardResponse;
          setBodegaData(dataB);
        }
      } catch (err) {
        setDashError(err instanceof Error ? err.message : t("Error cargando dashboards"));
      } finally {
  ...
  return (
    <MainLayout>
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between mb-6">
        <div>
          <p className="text-sm uppercase tracking-wide text-[#4B6B8A] font-semibold">
            {t("Dashboard de crecimiento sostenible")}
          </p>
          <h2 className="text-3xl font-extrabold text-[#1A334B]">{t("Vision general comercial")}</h2>
          <p className="text-gray-600">
            {t("Ventas, leads, conversion y pipeline en un solo panel para analisis semanal y optimizacion.")}
          </p>
        </div>
        <div className="flex flex-col items-start gap-2">
          ...
          <div className="text-xs text-gray-500">
            {t("Filtro activo")}:{" "}
            {rango === "semana" ? t("Semana actual") : rango === "30" ? t("Ultimos 30 dias") : t("Historico")}
          </div>
        </div>
      </div>
      ...
      {loadingDash && (
        <div className="mb-4 bg-[#F4F8FD] border border-[#D9E7F5] text-[#1A334B] px-4 py-3 rounded-lg text-sm">
          {t("Cargando paneles con datos reales...")}
        </div>
      )}
        ...
        <section className="mb-6 space-y-4">
              ...
              <div className="flex gap-2 flex-wrap mt-2">
                {adminData.clientesPorEstado.map((c) => (
                  <span key={c.estado} className="px-3 py-1 rounded-full text-[11px] bg-emerald-50 text-emerald-700">
                    {t(String(c.estado))}: {c.total}
                  </span>
                ))}
              </div>
            </div>
            <div className="bg-white border border-[#D9E7F5] rounded-xl p-4 shadow-sm">
              <p className="text-xs text-gray-500">{t("Cotizaciones")}</p>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-4">
            <div className="flex items-center justify-between mb-3">
              <div>
                <p className="text-xs text-gray-500 uppercase font-semibold">{t("Vista admin / supervisor")}</p>
                <p className="text-lg font-bold text-[#1A334B]">{t("Vendedores, leads y clientes")}</p>
                <p className="text-xs text-gray-500">
                  {t("Actualizado")}: {new Date(adminData.ultimaActualizacion).toLocaleString()}
                </p>
              </div>
            </div>
            <div className="overflow-auto">
              <table className="w-full min-w-[900px]">
                <thead className="text-left bg-[#F5FAFF] text-[#1A334B] text-sm">
                  <tr>
                    <th className="py-3 px-4">{t("Vendedor / Rol")}</th>
                    <th className="py-3 px-4">{t("Leads")}</th>
                    <th className="py-3 px-4">{t("Clientes")}</th>
                    <th className="py-3 px-4">{t("Cotizaciones")}</th>
                    <th className="py-3 px-4 w-[260px]">{t("Recientes")}</th>
                  </tr>
                </thead>
                <tbody>
                  {adminData.vendors.map((v) => (
                    <tr key={v.id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-3 px-4 text-sm">
                        <p className="font-semibold">
                          {v.cotizaciones.total} {t("cotizaciones")}
                        </p>
                        <div className="flex gap-1 flex-wrap mt-1">
                          ...
                          {v.recientes.leads.map((l) => (
                            <div key={l.id} className="flex justify-between gap-2 border border-[#E6F0FB] rounded-lg px-2 py-1">
                              <div>
                                ...
                                <p className="text-[11px] text-gray-500">{t(String(l.estado))}</p>
                              </div>
                              <span className="text-[11px] text-[#1A6CD3]">{t("Lead")}</span>
                            </div>
                          ))}
                          {v.recientes.cotizaciones.map((c) => (
                            <div key={c.id} className="flex justify-between gap-2 border border-amber-100 rounded-lg px-2 py-1">
                              <div>
                                ...
                                <p className="text-[11px] text-amber-700">{t(String(c.etapa))}</p>
                              </div>
                              <span className="text-[11px] text-amber-700">{t("Cot")}</span>
                            </div>
                          ))}
                        </div>
                      </td>
      ...
      {(isBodeguero || isAdmin) && bodegaData && (
        <section className="mb-6 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-gray-500 uppercase font-semibold">{t("Panel bodeguero")}</p>
              <p className="text-lg font-bold text-[#1A334B]">{t("Seguimiento de cotizaciones y entregas")}</p>
              <p className="text-xs text-gray-500">
                {t("Actualizado")}: {new Date(bodegaData.ultimaActualizacion).toLocaleString()}
              </p>
            </div>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {bodegaData.cotizacionesPorEtapa.map((c) => (
              <div key={c.etapa} className="bg-white border border-[#D9E7F5] rounded-xl p-3 shadow-sm">
                <p className="text-xs text-gray-500">{t(String(c.etapa))}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-4">
              <p className="text-sm font-bold text-[#1A334B] mb-2">{t("Entregas programadas")}</p>
              <div className="space-y-2 max-h-[320px] overflow-auto">
                {bodegaData.entregas.length === 0 && (
                  <p className="text-xs text-gray-500">{t("Sin entregas agendadas")}</p>
                )}
                {bodegaData.entregas.map((e) => (
                  <div key={e.id} className="border border-amber-100 rounded-lg px-3 py-2 text-sm">
                    ...
                    <p className="text-xs text-amber-700">{t(String(e.etapa))}</p>
                    {e.entregaProgramada && (
                      <p className="text-[11px] text-gray-600">
                        {t("Entrega")}: {new Date(e.entregaProgramada).toLocaleString()}
                      </p>
                    )}
                    {e.comentarios && <p className="text-[11px] text-gray-600">{e.comentarios}</p>}
                    {e.imagenUrl && (
                      <p className="text-[11px] text-[#1A6CD3]">{t("Adjunto disponible")}</p>
                    )}
                  </div>
                ))}
              </div>
            ...
            <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-4">
              <p className="text-sm font-bold text-[#1A334B] mb-2">{t("Ultimas cotizaciones (bodega)")}</p>
              <div className="space-y-2 max-h-[320px] overflow-auto">
                {bodegaData.recientes.length === 0 && (
                  <p className="text-xs text-gray-500">{t("Sin registros")}</p>
                )}
                {bodegaData.recientes.map((c) => (
                  <div key={c.id} className="border border-[#E6F0FB] rounded-lg px-3 py-2 text-sm">
                    ...
                    <p className="text-[11px] text-gray-600">{t(String(c.etapa))}</p>
                    {c.entregaProgramada && (
                      <p className="text-[11px] text-gray-500">
                        {t("Entrega")}: {new Date(c.entregaProgramada).toLocaleString()}
                      </p>
                    )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 mb-8">
        {kpis.map((kpi) => (
          <KPI
            ...
            {...kpi}
            title={t(kpi.title)}
            hint={kpi.hint ? t(kpi.hint) : undefined}
          />
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 mb-8">
        <div className="xl:col-span-2 bg-white border border-[#D9E7F5] rounded-2xl shadow-sm p-5">
          <div className="flex items-center justify-between mb-3">
            <div>
              <h3 className="text-xl font-bold text-[#1A334B]">{t("Ventas x mes")}</h3>
              <p className="text-sm text-gray-500">{t("Comparativo actual vs mes previo")}</p>
            </div>
          ...
          <div className="flex gap-3 items-end h-56">
            {ventasMes.map((item) => (
              <div key={item.mes} className="flex-1 flex flex-col gap-1 items-center">
                ...
                <p className="text-xs font-semibold text-gray-700">{t(item.mes)}</p>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white border border-[#D9E7F5] rounded-2xl shadow-sm p-5">
          <div className="flex items-center justify-between mb-3">
            <div>
              <h3 className="text-xl font-bold text-[#1A334B]">{t("Leads x semana")}</h3>
              <p className="text-sm text-gray-500">{t("Tendencia semanal")}</p>
            </div>
            <span className="text-xs font-semibold text-[#1A6CD3] bg-[#E6F0FB] px-2 py-1 rounded-full">
              {t("+8% respecto a la anterior")}
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-8">
        <div className="bg-white border border-[#D9E7F5] rounded-2xl shadow-sm p-5">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xl font-bold text-[#1A334B]">{t("Pipeline de ventas")}</h3>
            <span className="text-xs text-gray-500">{t("Etapas con tasa acumulada")}</span>
          </div>
          <div className="space-y-3">
            {pipeline.map((step, idx) => (
              <div key={step.etapa} className="flex items-center gap-3">
                <div className="w-28">
                  <p className="text-sm font-semibold text-[#1A334B]">{t(step.etapa)}</p>
        ...
        <div className="bg-white border border-[#D9E7F5] rounded-2xl shadow-sm p-5">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xl font-bold text-[#1A334B]">{t("Razones de perdida")}</h3>
            <span className="text-xs text-gray-500">{t("Top motivos")}</span>
          </div>
          <div className="space-y-3">
            {razonesPerdida.map((r) => (
              <div key={r.motivo}>
                <div className="flex items-center justify-between mb-1">
                  <p className="text-sm font-semibold text-[#1A334B]">{t(r.motivo)}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
        {resumenGeneral.map((item) => (
          ...
          >
            <p className="text-xs uppercase tracking-wide text-white/80 font-semibold">{t(item.titulo)}</p>
            <p className="text-3xl font-extrabold">{item.valor}</p>
            <p className="text-sm text-white/90 mt-1">{t(item.detalle)}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-8">
        <section className="bg-white border border-[#D9E7F5] rounded-2xl shadow-sm p-5">
          <div className="flex items-center justify-between mb-3">
            <div>
              <h3 className="text-xl font-bold text-[#1A334B]">{t("Comparativa de vendedores")}</h3>
              <p className="text-sm text-gray-500">{t("Leads, clientes y conversion por responsable")}</p>
            </div>
            <span className="text-xs font-semibold text-sky-700 bg-sky-50 px-2 py-1 rounded-full">
              {t("Admin / Super admin / Supervisor")}
            </span>
          </div>
            ...
            {vendedoresKpi.map((v) => (
              <div key={v.nombre} className="py-3 flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-gray-500">{t("Leads")}: {v.leads} ┬À {t("Clientes")}: {v.clientes}</p>
                </div>
                <div className="flex-1 max-w-[220px]">
                  ...
                  <p className="text-[11px] text-gray-500 mt-1">{t("Conversion")} {v.conversion}% ┬À {t("Ventas")} {v.ventas}</p>
                </div>
        ...
        <section className="bg-white border border-[#D9E7F5] rounded-2xl shadow-sm p-5">
          <div className="flex items-center justify-between mb-3">
            <div>
              <h3 className="text-xl font-bold text-[#1A334B]">{t("Operaciones Bodega")}</h3>
              <p className="text-sm text-gray-500">{t("Cotizaciones, transito y entregas por bodeguero")}</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {bodeguerosKpi.map((b) => (
              <div key={b.nombre} className="border border-[#E6F0FB] rounded-xl p-3 shadow-inner bg-gradient-to-br from-white to-[#F7FAFF]">
                ...
                <p className="text-xs text-gray-500 mb-2">{t("Dia")}: {b.cotizacionesDia} ┬À {t("Semana")}: {b.semana}</p>
                <div className="flex items-center justify-between text-xs text-gray-700">
                  <span>{t("En transito")}</span>
      ...
      <section className="bg-white border border-[#D9E7F5] rounded-2xl shadow-sm p-5">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-xl font-bold text-[#1A334B]">{t("Oportunidades destacadas")}</h3>
          <span className="text-xs font-semibold text-emerald-700 bg-emerald-50 px-2 py-1 rounded-full">
            {t("En seguimiento")}
          </span>
        </div>
          ...
          <table className="w-full min-w-[480px]">
            <thead className="bg-[#F5FAFF] text-[#1A334B] text-left">
              <tr>
                ...
                <th className="py-3 px-4 text-sm">{t("Etapa")}</th>
                <th className="py-3 px-4 text-sm">{t("Monto")}</th>
                <th className="py-3 px-4 text-sm">{t("Dias en etapa")}</th>
              </tr>
            </thead>
            <tbody>
              {oportunidades.map((op) => (
                <tr key={op.cliente} className="border-t border-gray-200 hover:bg-gray-50 transition">
                  ...
                  <td className="py-3 px-4 text-sm text-gray-700">{t(op.etapa)}</td>
