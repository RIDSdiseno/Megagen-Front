...
export default function TerrainHistoryPage() {
  ...
  const exportCsv = () => {
    const rows = [
      ["id", "codigo", "cliente", "total", "etapa", "responsable", "fecha", "entregaProgramada", "resumen"],
      ...filtradas.map((c) => [
        c.id,
        c.codigo,
        c.cliente,
  ...
  const exportPdf = () => {
      ...
      .map(
        (c) => `
          <tr>
            ...
            <td style="padding:6px 8px;border:1px solid #e5e7eb;">${c.etapa}</td>
            <td style="padding:6px 8px;border:1px solid #e5e7eb;">${c.vendedorEmail || ""}</td>
            <td style="padding:6px 8px;border:1px solid #e5e7eb;">${c.entregaProgramada || "—"}</td>
          </tr>`
      )
      .join("");
    ...
    win.document.write(`
      <html>
        <head>
          <title>Informe de Cotizaciones</title>
          <style>
        ...
        <body>
          ...
          <table>
            <thead>
              <tr>
                <th>Código</th><th>Cliente</th><th>Total</th><th>Etapa</th><th>Responsable</th><th>Entrega</th>
              </tr>
            </thead>
            <tbody>${rows}</tbody>
          </table>
    ...
    return (
      <MainLayout>
        <div className="bg-white border border-amber-200 rounded-2xl p-6 shadow-sm">
          <p className="text-lg font-bold text-[#1A334B] mb-1">Acceso restringido</p>
          <p className="text-sm text-gray-600">Solo administradores o supervisores pueden revisar y exportar informes de cotizaciones.</p>
        </div>
      </MainLayout>
    );
  }
    ...
    <MainLayout>
      <div className="flex flex-col gap-3 mb-6 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <h2 className="text-3xl font-extrabold text-[#1A334B]">Historial de cotizaciones</h2>
          <p className="text-gray-600 text-sm">
            Revisa y exporta las cotizaciones generadas por vendedores y bodegueros, filtrando por cliente, responsable y etapa.
          </p>
        </div>
        ...
        {(Object.keys(resumenEtapa) as Etapa[]).map((etapa) => (
          ...
          >
            <p className="text-xs font-semibold text-gray-600 flex items-center justify-between">
              {etapa}
              {etapaFiltro === etapa && <span className="text-[10px] px-2 py-0.5 bg-white/60 rounded-full">Filtro</span>}
            </p>
        ...
        <div className="flex flex-wrap gap-3 items-center">
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[#F4F8FD] border border-[#D9E7F5] text-gray-700">
            ...
            <input
              ...
              placeholder="Buscar por cliente, código o resumen"
      ...
      {loading ? (
        <div className="flex items-center gap-2 text-[#1A334B] text-sm">
          <RefreshCw size={16} className="animate-spin" /> Cargando cotizaciones...
        </div>
      ) : (
            ...
            <tbody>
              {filtradas.map((c) => (
                ...
                >
                  ...
                  <td className="py-3 px-4 text-sm text-gray-700">{c.vendedorEmail || "Sin responsable"}</td>
                  <td className="py-3 px-4 text-sm text-gray-700">
                    {c.entregaProgramada ? new Date(c.entregaProgramada).toLocaleString() : "Sin programar"}
                  </td>
                  <td className="py-3 px-4 text-sm text-gray-700 truncate max-w-[240px]" title={c.resumen || c.comentarios}>
                    {c.resumen || c.comentarios || "—"}
                  </td>
                </tr>
              ))}
              {filtradas.length === 0 && (
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-2xl max-h-[80vh] overflow-auto">
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-xs uppercase tracking-wide text-[#4B6B8A] font-semibold">Detalle de cotización</p>
                <h3 className="text-xl font-bold text-[#1A334B]">#{seleccion.codigo} - {seleccion.cliente}</h3>
              </div>
              <button onClick={() => setSeleccion(null)} className="text-gray-500 hover:text-gray-700 text-sm">Cerrar</button>
            </div>
            <div className="grid grid-cols-2 gap-3 text-sm text-gray-700">
              <p><strong>Total:</strong> {seleccion.total}</p>
              <p><strong>Etapa:</strong> {seleccion.etapa}</p>
              <p><strong>Responsable:</strong> {seleccion.vendedorEmail || "Sin responsable"}</p>
              <p><strong>Entrega:</strong> {seleccion.entregaProgramada ? new Date(seleccion.entregaProgramada).toLocaleString() : "Sin programar"}</p>
              <p className="col-span-2"><strong>Resumen:</strong> {seleccion.resumen || "N/A"}</p>
              <p className="col-span-2"><strong>Comentarios:</strong> {seleccion.comentarios || "N/A"}</p>
              <div className="col-span-2">
                <p className="font-semibold text-[#1A334B] mb-1 flex items-center gap-2"><FileText size={14}/> Historial</p>
                <div className="space-y-1">
                  {seleccion.historico.length === 0 && <p className="text-xs text-gray-500">Sin eventos registrados.</p>}
