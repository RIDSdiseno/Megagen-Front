...
type LeadEstadoLabel = "Nuevo" | "Cotizando" | "En Proceso" | "Confirmado";
type LeadEstadoApi = "NUEVO" | "COTIZANDO" | "EN_PROCESO" | "CONFIRMADO" | "ENTREGADO";
type LeadEstado = LeadEstadoLabel | LeadEstadoApi;
...
const estadoOrden: LeadEstadoLabel[] = ["Nuevo", "Cotizando", "En Proceso", "Confirmado"];
const origenes: Lead["origen"][] = ["WhatsApp", "Llamada", "Correo", "Instagram", "Web"];

const normalizeLeadEstado = (estado: Lead["estado"]): LeadEstadoLabel => {
  ...
  if (raw === "CONFIRMADO" || raw === "ENTREGADO") return "Confirmado";
  return "Nuevo";
};
...
export default function Leads() {
  ...
  const [estadoFiltro, setEstadoFiltro] = useState<LeadEstadoLabel | "Todos">("Todos");
  ...
  const resumen = useMemo(
    () => estadoOrden.map(estado => ({
      estado,
      total: leadsVisibles.filter(l => normalizeLeadEstado(l.estado) === estado).length
    })),
    [leadsVisibles]
  );
  ...
  const filtered = useMemo(() => {
    ...
    return leadsVisibles.filter(l => {
      ...
      const matchEstado = estadoFiltro === "Todos" ? true : normalizeLeadEstado(l.estado) === estadoFiltro;
  ...
  const crearRecordatorio = () => {
    ...
    addEvent({
      ...
      correo: agendaLead.correo,
      estado: normalizeLeadEstado(agendaLead.estado),
      resumen: agendaLead.proximoPaso || agendaLead.nota || "",
      ownerEmail: vendorById[agendaLead.vendorId]?.email || user?.email,
    });
    pushNotification({
      title: t("Contacto con {nombre}", { nombre: agendaLead.nombre }),
      detail: `${t("{canal} programado el {fecha}", { canal: t(agendaCanal), fecha: fechaLegible })}${
        agendaNota ? ` - ${agendaNota}` : ""
      }`,
      time: fechaLegible,
    });
    setAgendaLead(null);
    ...
    <MainLayout>
      <div className="flex flex-col gap-4 mb-6">
        ...
        <div>
          ...
          <h2 className="text-3xl font-extrabold text-[#1A334B]">{t("Leads / Clientes")}</h2>
          <p className="text-gray-600 text-sm">{t("Comunicate por mensaje, correo o llamada y da seguimiento rapido.")}</p>
          <p className="text-xs text-amber-700 bg-amber-50 border border-amber-100 rounded-lg px-3 py-2 inline-block mt-1">
            {t("Roles activos")}: {(user?.roles || []).join(", ") || t("Invitado")} - {t("Solo admin/supervisor asigna, vendedor gestiona su propia cartera.")}
          </p>
        </div>
          <button
            ...
            return (
              ...
              >
                <p className="text-xs font-semibold text-gray-500 flex items-center justify-between">
                  {t(item.estado)}
                  {active && <span className="px-2 py-0.5 rounded-full bg-[#E6F0FB] text-[#1A6CD3] text-[10px]">{t("Filtro")}</span>}
                </p>
        ...
        {estadoFiltro !== "Todos" && (
          <div className="flex items-center gap-2 text-sm mt-2">
            <span className="text-[#1A334B] font-semibold">{t("Filtro por estado")}:</span>
            <span className="px-3 py-1 rounded-full bg-[#E6F0FB] text-[#1A6CD3] text-xs font-semibold">{t(estadoFiltro)}</span>
            <button
      ...
      {isAdminLike && (
        <div className="bg-white border border-gray-200 rounded-xl p-3 shadow-sm flex flex-col gap-3">
          <div className="flex items-center gap-2 flex-wrap">
            <div className="text-sm font-semibold text-[#1A334B]">{t("Seleccionar vendedor")}</div>
            <button
            ...
            {resumenPorVendedor.map((v) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-xs text-gray-600">{t("{total} leads", { total: v.total })}</p>
                </button>
              );
            })}
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {resumenOrigen.map((item) => (
            ...
            >
              ...
              <p className="text-sm font-bold text-[#1A334B]">{t(item.origen)}</p>
      ...
      <div className="bg-white p-4 border border-gray-200 rounded-xl flex flex-col lg:flex-row gap-3 items-start lg:items-center mb-6 shadow-sm">
        <div className="flex items-center gap-3 w-full lg:w-1/2">
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[#F4F8FD] border border-[#D9E7F5] text-gray-600 flex-1">
            ...
            <input 
              type="text"
              placeholder={t("Buscar cliente por nombre, correo o telefono...")}
        ...
        <div className="flex flex-wrap gap-2 w-full lg:w-1/2">
          <EstadoChip estado="Todos" activo={estadoFiltro === "Todos"} onClick={() => setEstadoFiltro("Todos")} t={t} />
          {estadoOrden.map(estado => (
            <EstadoChip key={estado} estado={estado} activo={estadoFiltro === estado} onClick={() => setEstadoFiltro(estado)} t={t} />
          ))}
          <div className="w-full flex flex-wrap gap-2 mt-2">
            <EstadoChip
              estado="Todos"
              activo={origenFiltro === "Todos"}
          ...
          {filtered.map(lead => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-xs text-gray-500">{t("Cliente #{id}", { id: lead.id })}</p>
                  ...
                  <p className="text-xs text-[#1A6CD3] font-semibold mt-1">
                    {t("Vendedor")}: {vendorById[lead.vendorId]?.nombre || vendorById[lead.vendorId]?.email} - {t("Origen")}: {t(lead.origen)}
                  </p>
                </div>
                <EstadoBadge estado={lead.estado} t={t} />
              </div>

              <div className="mt-3 text-sm text-gray-700 bg-[#F7FAFF] border border-[#D9E7F5] rounded-xl p-3">
                <p className="font-semibold text-[#1A334B]">{t("Proximo paso")}</p>
                <p className="text-gray-600">{lead.proximoPaso}</p>
                <p className="text-xs text-gray-500 mt-2">{t("Ingreso")}: {lead.fechaIngreso}</p>
                {lead.nota && <p className="text-xs text-gray-500">{t("Nota")}: {lead.nota}</p>}
              </div>
              ...
              <AccionBoton
                onClick={() => {
                  pushNotification({
                    title: t("Recordar: {nombre}", { nombre: lead.nombre }),
                    detail: lead.proximoPaso || lead.nota || t("Revisar comentarios del cliente"),
          ...
          <table className="w-full min-w-[900px]">
            <thead className="bg-[#F5FAFF] text-[#1A334B] text-left">
              <tr>
                ...
                <th className="py-3 px-4 text-sm">{t("Ingreso")}</th>
                <th className="py-3 px-4 text-sm">{t("Proximo paso")}</th>
            ...
            <tbody>
              {filtered.map((lead) => (
                <tr key={lead.id} className="border-t border-gray-200 hover:bg-gray-50 transition">
                  ...
                  <td className="py-3 px-4 text-sm text-gray-700">{t(lead.origen)}</td>
                  ...
                  <td className="py-3 px-4 text-sm text-gray-700 max-w-[280px]">
                    ...
                    {lead.nota && <p className="text-xs text-gray-500 mt-1">{t("Nota")}: {lead.nota}</p>}
                  </td>
                  <td className="py-3 px-4">
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-xl">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-bold text-[#1A334B]">{t("Nuevo cliente")}</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <Input label={t("Telefono")} value={nuevo.telefono} onChange={(v) => setNuevo({ ...nuevo, telefono: v })} />
              <Input label={t("Proximo paso")} value={nuevo.proximoPaso} onChange={(v) => setNuevo({ ...nuevo, proximoPaso: v })} />
              <Input label={t("Nota")} value={nuevo.nota ?? ""} onChange={(v) => setNuevo({ ...nuevo, nota: v })} />
              ...
              <div className="flex flex-col gap-1">
                <span className="text-xs font-semibold text-[#1A334B]">{t("Origen")}</span>
                <select
                  className="border border-[#D9E7F5] rounded-lg px-3 py-2 text-sm text-gray-700"
                  value={nuevo.origen}
      ...
      {agendaLead && (
        <Modal onClose={() => setAgendaLead(null)}>
          <div className="p-5 space-y-3">
            <h3 className="text-xl font-bold text-[#1A334B]">{t("Agendar contacto")}</h3>
            ...
            <label className="flex flex-col gap-1 text-xs text-[#1A334B]">
              <span className="font-semibold">{t("Canal")}</span>
              <select
                value={agendaCanal}
              ...
              >
                ...
                <option value="Correo">{t("Correo")}</option>
            ...
            <div className="text-xs text-emerald-700 bg-emerald-50 border border-emerald-100 rounded-lg px-3 py-2">
              {t("Se notificara por panel y se sugiere enviar el mensaje por {canal}.", { canal: agendaCanal })}
            </div>
      ...
      {detalle && (
        <Modal onClose={() => setDetalle(null)}>
          <div className="p-5 space-y-3">
            <h3 className="text-xl font-bold text-[#1A334B]">{t("Detalle lead #{id}", { id: detalle.id })}</h3>
            <p className="text-sm text-gray-700"><strong>{t("Nombre")}:</strong> {detalle.nombre}</p>
            <p className="text-sm text-gray-700"><strong>{t("Correo")}:</strong> {detalle.correo}</p>
            <p className="text-sm text-gray-700"><strong>{t("Telefono")}:</strong> {detalle.telefono}</p>
            <p className="text-sm text-gray-700"><strong>{t("Origen")}:</strong> {t(detalle.origen)}</p>
            <p className="text-sm text-gray-700"><strong>{t("Estado")}:</strong> {t(normalizeLeadEstado(detalle.estado))}</p>
            <p className="text-sm text-gray-700"><strong>{t("Ingreso")}:</strong> {detalle.fechaIngreso}</p>
            <p className="text-sm text-gray-700"><strong>{t("Proximo paso")}:</strong> {detalle.proximoPaso}</p>
            {detalle.nota && <p className="text-sm text-gray-700"><strong>{t("Nota")}:</strong> {detalle.nota}</p>}
...
function EstadoChip({
  ...
  return (
    <button onClick={onClick} className={`${base} ${activo ? activeStyles : idleStyles}`}>
      {labelPrefix ? `${t(labelPrefix)}: ${t(estado)}` : t(estado)}
    </button>
  );
}

function EstadoBadge({ estado, t }: { estado: Lead["estado"]; t: (key: string) => string }) {
  const label = normalizeLeadEstado(estado);
  const isCliente = label === "Confirmado";
